...
#             self.scheduler = optim.lr_scheduler.ReduceLROnPlateau(self.optimizer, 'min', patience=3)
            self.scheduler = optim.lr_scheduler.CyclicLR(self.optimizer, base_lr= lr/10., max_lr=lr, step_size_up=10)
...
#                 transforms.RandomVerticalFlip(p=0.5),  ## Removing as it doesn't apply same to both the images
    ...
    # train the model
    optimizer_name = 'sgd'
    model_path = f"./saved_models/regresn_{optimizer_name}_{args.lr}_Cycle_epoch_{args.ep}_{args.loss_fn}_scale_{args.ls}_es_{args.es}_ProbSc1.pth"
