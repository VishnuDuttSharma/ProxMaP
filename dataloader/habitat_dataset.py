...
class HabitatDataset(data.Dataset):
    def __init__(self, filename='./habitat_data_description.csv', 
                 transform=None, 
                 input_dir='./occmap_data/', 
                 target_dir='./occmap_data/', 
                 mode='test', 
                 odds_to_prob=True, 
                 prob_scale=10., 
                 count_to_odd=True, 
    ...
    def __getitem__(self, index):
        ...
        tgt_img = np.load(f'{self.target_dir}/{self.filepaths[index].replace("inp", "gt")}.npy')
